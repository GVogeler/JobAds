...
STARTING_YEAR = 1874    # first year of the newspaper we want to download
ENDING_YEAR = 1943      # last year of the newspaper we want to download
NEWSPAPER_TAG = "nwb"   # ANNO tag of the newspaper we want to download
...
for year in range(STARTING_YEAR, ENDING_YEAR+1):
    ...
    for url in tqdm.tqdm(days_urls):
        ...
        if len(pages_urls) >= 20:
            target_pages = pages_urls[4:]
        else:
            if (year < 1902) or (year == 1902 and MONTH < 9):
                target_pages = pages_urls[5:]
            elif (year == 1902 and MONTH >= 9) or year >= 1903:
