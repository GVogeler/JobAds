...
STARTING_YEAR = 1881    # first year of the newspaper we want to download
ENDING_YEAR = 1938      # last year of the newspaper we want to download
NEWSPAPER_TAG = "fst"   # ANNO tag of the newspaper we want to download
...
for year in range(STARTING_YEAR, ENDING_YEAR+1):
    ...
    for url in tqdm.tqdm(days_urls):
        ...
        if 1881 <= year < 1892:
        ...
        elif (1899 <= year < 1908) or (year == 1908 and MONTH < 4):
            target_pages = pages_urls[5:]
        elif (year == 1908 and MONTH >= 4) or (1908 < year < 1912):
            if DAY == "Sa":
            ...
            if DAY == "So":
                target_pages = pages_urls[10:]
            elif DAY == "Mo" and len(pages_urls) == 4:
            ...
            if DAY in WEEKEND:
                ...
                target_pages = pages_urls[4:]	
        elif (1918 <= year < 1920) or (year == 1920 and MONTH < 10):
            if DAY == "So":
                target_pages = pages_urls[4:]
            else:
                target_pages = pages_urls[2:]
        elif (year == 1920 and MONTH >= 10) or (year == 1921 and MONTH < 5):
            if DAY == "Sa":
                target_pages = pages_urls[4:]
            else: 
                target_pages = pages_urls[2:]
        elif (year == 1921 and MONTH >= 5) or (1921 < year < 1926):
            if DAY == "Sa":
