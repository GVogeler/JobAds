...
STARTING_YEAR = 1860    # first year of the newspaper we want to download
ENDING_YEAR = 1911      # last year of the newspaper we want to download
NEWSPAPER_TAG = "vtl"   # ANNO tag of the newspaper we want to download
...
for year in range(STARTING_YEAR, ENDING_YEAR+1):
    ...
    for url in tqdm.tqdm(days_urls):
        ...
        if (year < 1866) or (year == 1866 and MONTH < 12):
            target_pages = pages_urls[3:]
        elif (year == 1866 and MONTH == 12) or (1866 < year < 1880):
            if DAY == "Mo":
                target_pages = []
            else: 
            ...
            if DAY == "Mo":
                target_pages = pages_urls[2:4]
            else: target_pages = pages_urls[5:]
        elif 1893 <= year < 1908:
            if DAY == "Mo":
