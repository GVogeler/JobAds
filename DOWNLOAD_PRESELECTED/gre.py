...
STARTING_YEAR = 1868    # first year of the newspaper we want to download
ENDING_YEAR = 1929      # last year of the newspaper we want to download
NEWSPAPER_TAG = "gre"   # ANNO tag of the newspaper we want to download
...
for year in range(STARTING_YEAR, ENDING_YEAR+1):
    ...
    for url in tqdm.tqdm(days_urls):
        ...
        if year < 1880:
            ...
            if len(pages_urls) == 12 or len(pages_urls) == 14:
                target_pages = pages_urls[3:-4]
            else:
                target_pages = pages_urls[3:] 
        else:
