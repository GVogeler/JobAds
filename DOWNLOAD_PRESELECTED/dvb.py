...
ENDING_YEAR = 1922      # last year of the newspaper we want to download
NEWSPAPER_TAG = "dvb"   # ANNO tag of the newspaper we want to download
    ...
    for url in tqdm.tqdm(days_urls):
        ...
        if DAY == "Mo":
            ...
            if year <= 1890:
                ...
                target_pages.extend(pages_urls[15:])
                target_pages = list(set(target_pages))
            elif year >= 1915:
                target_pages = pages_urls[-1:]
        elif len(pages_urls) >= 30:
            target_pages = pages_urls[20:]
        elif len(pages_urls) < 30:
            target_pages = pages_urls[-9:-8]
            target_pages.extend(pages_urls[-7:-6])
            target_pages.extend(pages_urls[-5:-4])
