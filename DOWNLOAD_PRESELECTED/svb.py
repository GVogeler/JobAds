...
STARTING_YEAR = 1871    # first year of the newspaper we want to download
ENDING_YEAR = 1942      # last year of the newspaper we want to download
NEWSPAPER_TAG = "svb"   # ANNO tag of the newspaper we want to download
...
for year in range(STARTING_YEAR, ENDING_YEAR+1):
    ...
    for url in tqdm.tqdm(days_urls):
        ...
        elif (1882 <= year < 1921) or (year == 1921 and MONTH <= 11):
            target_pages = pages_urls[3:]
        elif (year == 1921 and MONTH == 12) or (1921 <= year < 1933) or (year == 1933 and MONTH < 6):
            if DAY not in WEEKEND:
                target_pages = pages_urls[-2:]
            elif DAY == "Sa":
                target_pages = pages_urls[7:]
        elif (year == 1933 and MONTH >= 6) or (1934 <= year < 1939) or (year == 1939 and MONTH < 9):
            if DAY not in WEEKEND:
                target_pages = pages_urls[-2:]
            elif DAY == "Sa":
                target_pages = pages_urls[17:]
        elif (year == 1939 and MONTH >= 9) or (1939 < year < 1941):
            if DAY not in WEEKEND:
